...
export function FullSidebar({ activeView, onNavigate, sidebarOpen, onToggle }: SidebarProps) {
                                    ...
                                    {item.hasDropdown ? (
                                        <>
                                            <button
                                                ...
                                                className={`flex items-center justify-between w-full p-3 text-sm font-medium rounded-lg transition-colors ${activeView === item.id
                                            ...
                                            {channelsOpen && (
                                                <ul className="ml-8 mt-1 space-y-1">
                                                    <li>
                                                        <button
                                                            onClick={() => { onNavigate('channels'); onToggle(); }}
                                                    ...
                                                    <li>
                                                        <button
                                                            onClick={() => { onNavigate('create-channel'); onToggle(); }}
                                                            className="w-full text-left p-2 text-sm text-gray-600 hover:bg-[#F8EDEB] rounded-lg"
                                                        >
                                                            + Create Channel
                                                        </button>
                        ...
                        <ul className="space-y-1">
                            {bottomItems.map((item) => (
                                <li key={item.id}>
                                    <button
                                        onClick={() => { onNavigate(item.id); onToggle(); }}
                                        className={`flex items-center w-full p-3 text-sm font-medium rounded-lg transition-colors ${activeView === item.id
                                                ? 'bg-[#FAE1DD] text-gray-900'
                                                : 'text-gray-700 hover:bg-[#F8EDEB]'
                                            }`}
                                    >
